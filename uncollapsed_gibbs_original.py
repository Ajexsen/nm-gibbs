...
class UncollapsedGibbs(MonteCarlo):
    ...
    def metropolis_hastings_K_new(self, object_index, singleton_features):
        ...
        # sample K_new from the metropolis hastings3 proposal distribution, i.e., a poisson distribution with mean \frac{\alpha}{N}
        ...
        A_new = numpy.vstack((self._A[[k for k in list(range(self._K)) if k not in singleton_features], :], A_temp));
        ...
        Z_new = numpy.hstack((self._Z[[object_index], [k for k in list(range(self._K)) if k not in singleton_features]], numpy.ones((len(object_index), K_temp))));
        ...
        if random.random() < prob_new:
            ...
            self._A = A_new;
            self._Z = numpy.hstack((self._Z[:, [k for k in list(range(self._K)) if k not in singleton_features]], numpy.zeros((self._N, K_temp))));
    ...
    def sufficient_statistics_A(self, observation_index=None):
        if observation_index == None:
            X = self._X;
            observation_index = list(range(self._D))
        else:
    ...
    def log_likelihood_X(self, X=None, Z=None, A=None):
        ...
        # compare
        self.log_like.append(log_likelihood)
    ...
    def log_likelihood_A(self):
        ...
        log_likelihood -= numpy.trace(numpy.dot((self._A - A_prior).transpose(), (self._A - A_prior))) * 1 / 2*(self._sigma_a ** 2);
