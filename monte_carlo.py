...
class MonteCarlo(object):
    ...
    def __init__(self, #real_valued_latent_feature=True,
        ...
        #
        assert(beta_hyper_parameter == None or type(beta_hyper_parameter) == tuple);
        self._beta_hyper_parameter = beta_hyper_parameter;
        ...
        self.log_n = 100
        self.log_c = 0
        self.log_like = [None for _ in range(100)]
    ...
    """
    def _initialize(self, data, alpha=1.0, beta=1.0, sigma_a=1.0, sigma_x=1.0, initial_Z=None, initial_A=None, A_prior=None, NP=0, isP=False, GAMMA=0, CAPACITY=0):
        self._counter = 0;
        ...
        self.C = CAPACITY
        self.STATE_CATACLYSM_INIT = numpy.zeros(shape=(self._N, self._K))
        self.PA = numpy.ones(shape=(self._N, self._K)) * (self._N * self.C) # Experimental
        self.PB = numpy.ones(shape=(self._N, self._K)) * (self._N * self.C) # Experimental
        self.G = GAMMA
    ...
    def initialize_Z(self):
        ...
        for i in range(1, self._N + 1):
            ...
            if i == 1:
                ...
                K_new = scipy.stats.poisson.rvs((self._alpha * self._beta) / (self._beta + i - 1));
    ...
    def sample_sigma(sigma_hyper_parameter, matrix):
        ...
        # sample_sigma_x(self, sigma_x_hyper_parameter): sample_sigma(self._sigma_x_hyper_parameter, self._X - numpy.dot(self._Z, self._A));

        assert(not sigma_hyper_parameter is None);
    ...
    def sample_alpha(self):
        ...
        logging.debug("Alpha {}".format(alpha_new))
        return alpha_new;
    
    def sample_beta(self):
        assert(self._beta_hyper_parameter != None);
        assert(type(self._beta_hyper_parameter) == tuple);
    
        (beta_hyper_a, beta_hyper_b) = self._beta_hyper_parameter;
        
        posterior_shape = beta_hyper_a + self._K;
        H_N = numpy.array([range(self._N)]) + 1.0;
        H_N = numpy.sum(1.0 / H_N);
        posterior_scale = 1.0 / (beta_hyper_b + H_N);
        
        #posterior_shape = beta_hyper_a + self._Z.sum();
        #posterior_scale = 1.0/(beta_hyper_b + self._N);
     
        #beta_new = scipy.stats.gamma.rvs(posterior_shape, scale=posterior_scale);
        beta_new = self._beta
        logging.debug("Beta {}".format(beta_new))
        return beta_new  
    ...
    def log_likelihood_Z(self):
        ...
        Z_h = list(Z_h);
        for k_h in set(Z_h):
            tmp = float(math.factorial(Z_h.count(k_h)))
            log_likelihood -= numpy.log(tmp);
            
        # compute the \sum_{k=1}^{K_+} \frac{(N-m_k)! (m_k-1)!}{N!}
        for k in list(range(self._K)):
            m_k = Z_h[k];
            temp_var = 1.0;
    ...
    def import_snapshot(self, directory, index):
        ...
        logging.debug "successfully import the snapshot from " + directory + " for iteration " + str(index) + "..."
    '''